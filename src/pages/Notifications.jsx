...
export default function Notifications() {
  ...
  const fetchNotifications = async () => {
    try {
      ...
      enqueueSnackbar('Erreur lors du chargement des notifications', { variant: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const generateNotifications = async () => {
    try {
      ...
      const data = await response.json();
      setNotifications(data);
      // No success notification for generation to avoid noise
    } catch (error) {
      console.error('Error generating notifications:', error);
      enqueueSnackbar('Erreur lors de la génération des notifications', { variant: 'error' });
    } finally {
      setLoading(false);
    }
  };
  ...
  const markAsRead = async (notificationId) => {
    try {
      ...
      enqueueSnackbar('Erreur lors du marquage comme lu', { variant: 'error' });
    }
  };
  ...
  const markAsUnread = async (notificationId) => {
    try {
      ...
      enqueueSnackbar('Erreur lors du marquage comme non lu', { variant: 'error' });
    }
  };
  ...
  const markAllAsRead = async () => {
    try {
      ...
      enqueueSnackbar('Erreur lors du marquage de toutes les notifications comme lues', { variant: 'error' });
    }
  };
  ...
  const deleteNotification = async (notificationId) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        enqueueSnackbar('Notification supprimée', { variant: 'info' });
      }
    } catch (error) {
  ...
  const clearAllNotifications = async () => {
    // Directly clear without confirmation
    try {
      const response = await fetch(`${API_URL}/notifications`, {
        method: 'DELETE'
      });
      ...
      if (response.ok) {
        setNotifications([]);
        enqueueSnackbar('Toutes les notifications ont été supprimées', { variant: 'success' });
      }
    } catch (error) {
      console.error('Error clearing notifications:', error);
      enqueueSnackbar('Erreur lors de la suppression de toutes les notifications', { variant: 'error' });
    }
  };
